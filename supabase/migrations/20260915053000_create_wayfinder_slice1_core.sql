...
create table wf_system.owners (
  ...
  constraint owners_auth_user_fk foreign key (auth_user_id)
...
create table wf_system.command_receipts (
  ...
  created_at timestamptz not null default now(),
  constraint command_receipts_owner_fk foreign key (owner_id)
...
create table wf_system.module_change_outbox (
  ...
  constraint module_change_outbox_owner_fk foreign key (owner_id)
    references wf_system.owners(id) on delete restrict,
  constraint module_change_outbox_command_fk foreign key (command_id)
    references wf_system.command_receipts(command_id) on delete restrict,
  constraint module_change_outbox_attempt_count_ck check (attempt_count >= 0)
);
...
create index module_change_outbox_command_idx
  on wf_system.module_change_outbox (command_id)
...
create table wf_direction.nodes (
  ...
  created_at timestamptz not null default now(),
  constraint nodes_owner_fk foreign key (owner_id)
    references wf_system.owners(id) on delete restrict,
...
create table wf_direction.node_versions (
  ...
  constraint node_versions_owner_fk foreign key (owner_id)
    references wf_system.owners(id) on delete restrict,
  constraint node_versions_actor_owner_fk foreign key (actor_owner_id)
    references wf_system.owners(id) on delete restrict,
  constraint node_versions_node_owner_fk foreign key (node_id, owner_id)
    references wf_direction.nodes(id, owner_id) on delete restrict,
...
create index nodes_current_version_idx
  on wf_direction.nodes (current_version_id)
  where current_version_id is not null;
create index node_versions_owner_idx
  on wf_direction.node_versions (owner_id);
create index node_versions_actor_owner_idx
  on wf_direction.node_versions (actor_owner_id)
...
create table wf_direction.edges (
  ...
  actor_owner_id uuid,
  recorded_at timestamptz not null default now(),
  constraint direction_edges_owner_fk foreign key (owner_id)
    references wf_system.owners(id) on delete restrict,
  constraint direction_edges_actor_owner_fk foreign key (actor_owner_id)
    references wf_system.owners(id) on delete restrict,
  constraint direction_edges_from_owner_fk foreign key (from_node_id, owner_id)
    references wf_direction.nodes(id, owner_id) on delete restrict,
  constraint direction_edges_to_owner_fk foreign key (to_node_id, owner_id)
    references wf_direction.nodes(id, owner_id) on delete restrict,
  ...
  constraint direction_edges_not_self_part_dependency_ck
    check (relation not in ('PART_OF','DEPENDS_ON') or from_node_id <> to_node_id)
);

create index direction_edges_lookup_idx
  on wf_direction.edges (owner_id, from_node_id, to_node_id, relation)
  where lifecycle_status = 'ACTIVE';
create index direction_edges_from_fk_idx on wf_direction.edges (from_node_id, owner_id);
create index direction_edges_to_fk_idx on wf_direction.edges (to_node_id, owner_id);
create index direction_edges_actor_owner_idx
  on wf_direction.edges (actor_owner_id) where actor_owner_id is not null;
...
create table wf_practice.practices (
  ...
  constraint practices_owner_fk foreign key (owner_id)
    references wf_system.owners(id) on delete restrict,
  constraint practices_id_owner_uq unique (id, owner_id),
  constraint practices_lifecycle_ck check (lifecycle_status in ('ACTIVE','RETRACTED'))
);
...
create table wf_practice.session_versions (
  ...
  actor_owner_id uuid,
  recorded_at timestamptz not null default now(),
  constraint session_versions_owner_fk foreign key (owner_id)
    references wf_system.owners(id) on delete restrict,
  constraint session_versions_actor_owner_fk foreign key (actor_owner_id)
    references wf_system.owners(id) on delete restrict,
  constraint session_versions_session_owner_fk foreign key (session_id, owner_id)
    references wf_practice.sessions(id, owner_id) on delete restrict,
  constraint session_versions_practice_owner_fk foreign key (practice_id, owner_id)
  ...
  constraint session_versions_duration_ck check (duration_seconds is null or duration_seconds > 0),
...
create unique index session_versions_one_active_idx
  ...
  where lifecycle_status = 'ACTIVE';
create index practices_owner_name_idx on wf_practice.practices (owner_id, name);
create index sessions_current_version_idx
  on wf_practice.sessions (current_version_id) where current_version_id is not null;
create index session_versions_owner_idx on wf_practice.session_versions (owner_id);
create index session_versions_practice_fk_idx
  on wf_practice.session_versions (practice_id, owner_id);
create index session_versions_actor_owner_idx
  on wf_practice.session_versions (actor_owner_id) where actor_owner_id is not null;
...
create table wf_evidence.links (
  ...
  actor_owner_id uuid,
  recorded_at timestamptz not null default now(),
  constraint evidence_links_owner_fk foreign key (owner_id)
    references wf_system.owners(id) on delete restrict,
  constraint evidence_links_actor_owner_fk foreign key (actor_owner_id)
    references wf_system.owners(id) on delete restrict,
  ...
  constraint evidence_links_lifecycle_ck check (lifecycle_status in ('ACTIVE','RETRACTED')),
...
create index evidence_links_source_idx
  on wf_evidence.links (owner_id, source_namespace, source_type, source_record_id, source_version_id)
  where lifecycle_status = 'ACTIVE';
create index evidence_links_target_idx
  on wf_evidence.links (owner_id, target_namespace, target_type, target_record_id, target_version_id, target_aspect)
  where lifecycle_status = 'ACTIVE';
create index evidence_links_actor_owner_idx
  on wf_evidence.links (actor_owner_id) where actor_owner_id is not null;
...
begin
  ...
  if not found then raise exception 'direction node current_version_id does not resolve'; end if;
  ...
  if old.lifecycle_status = 'RETRACTED' then
    if new.lifecycle_status is distinct from old.lifecycle_status
      ...
      raise exception 'retracted direction edge cannot reactivate';
        ...
        and new.lifecycle_status not in ('ACTIVE','RETRACTED') then
...
begin
  ...
  if not found then raise exception 'practice session current_version_id does not resolve'; end if;
