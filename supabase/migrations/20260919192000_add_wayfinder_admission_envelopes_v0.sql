...
create table wf_system.admission_envelopes (
  id uuid primary key,
  owner_id uuid not null references wf_system.owners(id) on delete cascade,
  planner_proposal_id text not null,
  ...
  constraint admission_envelopes_status_ck check (status in ('STAGED','APPLIED','REJECTED')),
  constraint admission_envelopes_expiry_ck check (expires_at > created_at)
);

create index admission_envelopes_owner_status_expiry_idx
  on wf_system.admission_envelopes(owner_id,status,expires_at);

create unique index admission_envelopes_owner_planner_proposal_uq
  on wf_system.admission_envelopes(owner_id,planner_proposal_id,semantic_fingerprint);
...
begin
  ...
  delete from wf_system.admission_envelopes
   where owner_id=v_owner and status='STAGED' and expires_at<clock_timestamp();
...
begin
  ...
  if v_row.status='STAGED' and v_row.expires_at<clock_timestamp() then
...
begin
  ...
  update wf_system.admission_envelopes
   ...
   where id=p_proposal_id and owner_id=v_owner and status='STAGED'
...
revoke all on table wf_system.admission_envelopes from public,anon,authenticated;
