...
begin
  ...
  select coalesce(
    pg_catalog.jsonb_agg(
      pg_catalog.jsonb_build_object(
        ...
        'intent_state', v.intent_state,
        'lifecycle_status', v.lifecycle_status,
        'recorded_at', v.recorded_at
      ) order by v.recorded_at, n.id
    ),
    '[]'::jsonb
  ) into v_nodes
  ...
  join wf_direction.nodes fn
    on fn.id = e.from_node_id and fn.owner_id = e.owner_id
  join wf_direction.node_versions fv
    on fv.id = fn.current_version_id and fv.node_id = fn.id and fv.owner_id = fn.owner_id
  join wf_direction.nodes tn
    on tn.id = e.to_node_id and tn.owner_id = e.owner_id
  ...
  where e.owner_id = v_owner
    ...
    and fv.lifecycle_status = 'ACTIVE'
    and tv.lifecycle_status = 'ACTIVE';

  return pg_catalog.jsonb_build_object(
    'nodes', v_nodes,
    'edges', v_edges,
    'storage_coverage', pg_catalog.jsonb_build_object(
...
-- ---------------------------------------------------------------------------
-- PRACTICE RECENT CURRENT SESSIONS
-- ---------------------------------------------------------------------------
...
begin
  ...
  where s.owner_id = v_owner
    ...
    and (
      (sv.occurred_to is null and sv.occurred_from >= p_from and sv.occurred_from < p_to)
      or
      (sv.occurred_to is not null and sv.occurred_from < p_to and sv.occurred_to > p_from)
    );
  ...
  return pg_catalog.jsonb_build_object(
    ...
    'storage_coverage', pg_catalog.jsonb_build_object(
      'phenomenon', 'wayfinder_stored_practice_session_records',
...
-- ---------------------------------------------------------------------------
-- EVIDENCE BY EXACT ACTION VERSION
-- ---------------------------------------------------------------------------
...
begin
  ...
  select v.lifecycle_status,
         (n.current_version_id = v.id and v.lifecycle_status = 'ACTIVE')
    into v_target_lifecycle, v_target_current
    from wf_direction.nodes n
    join wf_direction.node_versions v
      on v.node_id = n.id and v.owner_id = n.owner_id
   where n.id = p_target_action_id
     and n.owner_id = v_owner
     and n.kind = 'action'
     and v.id = p_target_version_id;
  ...
  select count(*) into v_total
   ...
   where l.owner_id = v_owner
     and l.target_namespace = 'direction'
     and l.target_type = 'action'
     and l.target_record_id = p_target_action_id
     and l.target_version_id = p_target_version_id
     and l.lifecycle_status = 'ACTIVE';

  select coalesce(
    pg_catalog.jsonb_agg(x.payload order by x.recorded_at desc, x.link_id),
    '[]'::jsonb
  ) into v_links
  from (
    select
      ...
      pg_catalog.jsonb_build_object(
        ...
        'source', pg_catalog.jsonb_build_object(
          'namespace', l.source_namespace,
          'type', l.source_type,
          'id', l.source_record_id,
          'version', l.source_version_id
        ),
        'target', pg_catalog.jsonb_build_object(
          'namespace', l.target_namespace,
          'type', l.target_type,
          'id', l.target_record_id,
          'version', l.target_version_id,
          'aspect', l.target_aspect
        ),
        'relation', l.relation,
        'reason', l.reason,
        'recorded_at', l.recorded_at,
        'source_current', case
          when l.source_namespace = 'practice' and l.source_type = 'session' then exists (
            select 1
              from wf_practice.sessions s
              join wf_practice.session_versions sv
                on sv.id = s.current_version_id
             ...
             where s.id = l.source_record_id
               ...
               and s.current_version_id = l.source_version_id
               and sv.lifecycle_status = 'ACTIVE'
          )
          else false
        end,
        ...
        'currently_usable', (
          v_target_current
          and l.source_namespace = 'practice'
          and l.source_type = 'session'
          and exists (
            select 1
              from wf_practice.sessions s
              join wf_practice.session_versions sv
  ...
  return pg_catalog.jsonb_build_object(
    'target', pg_catalog.jsonb_build_object(
      'namespace', 'direction',
      'type', 'action',
      'id', p_target_action_id,
      'version', p_target_version_id,
      'lifecycle_status', v_target_lifecycle,
      'current', v_target_current
    ),
    'links', v_links,
    'returned_count', pg_catalog.jsonb_array_length(v_links),
    'matching_record_count', v_total,
    'storage_coverage', pg_catalog.jsonb_build_object(
      'phenomenon', 'wayfinder_recorded_evidence_links_for_exact_target',
      'completeness', v_completeness,
      'reason', case when v_total > p_limit then 'RESULT_LIMIT' else null end,
      'evaluated_at', pg_catalog.clock_timestamp()
    )
...
revoke all on function public.wf_direction_current() from public, anon;
revoke all on function public.wf_practice_recent(timestamptz,timestamptz,integer) from public, anon;
revoke all on function public.wf_evidence_for_target(uuid,uuid,integer) from public, anon;

grant execute on function public.wf_direction_current() to authenticated;
grant execute on function public.wf_practice_recent(timestamptz,timestamptz,integer) to authenticated;
grant execute on function public.wf_evidence_for_target(uuid,uuid,integer) to authenticated;
