...
import { createNatalGeometryRuntime } from "../_shared/intelligence/natal-geometry-service.ts";
...
interface NatalGeometryRequestBody {
...
Deno.serve(async (req: Request) => {
  ...
  let body: NatalGeometryRequestBody = {};
  try {
    const text = await req.text();
    body = text.trim() ? (JSON.parse(text) as NatalGeometryRequestBody) : {};
  } catch {
  ...
  try {
    ...
    const runtime = createNatalGeometryRuntime();
    ...
    console.error("natal-geometry failure", error);
    return json(
      {
        error: "NATAL_GEOMETRY_FAILED",
        message: error instanceof Error ? error.message : "Unknown natal-geometry failure."
      },
      500
    );
  }
