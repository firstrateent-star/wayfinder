...
function semanticSystemPrompt() {
  return [
    ...
    "Context items marked kind=semantic_episode_candidate are transient prior-turn interpretations, never canonical facts. Use them to resolve ellipsis, corrections, and references when supported, but current-source meaning always outranks them and their presence never authorizes persistence.",
    "When a short current turn supplies a field or refinement for exactly one prior episode event (for example a body-area focus after 'I worked out today'), emit the current-source refinement using that prior event's known concept and cite the episode ref. Do not invent a new miscellaneous concept merely because the fragment is short. Preserve ambiguity instead when more than one prior event could be the target.",
    "Do not replay a semantic_episode_candidate as a fresh current-turn node merely to serve as a correction, negation, repetition, or reference target. Current graph nodes must express current-source meaning. Link prior-turn meaning through its full episode context ref in contextRefs/references. Candidate graph edges may connect only nodes emitted in the current graph; never put a prior-turn local candidate id in fromCandidateId or toCandidateId.",
    ...
    "Use STRENGTH_SESSION_STANDARD only when the player establishes a recurring normative strength-training frequency such as 'at least 3 strength sessions per week'. Preserve the explicit weekly target in attributes such as targetSessions or sessionsPerWeek. A past workout count, an occurred workout, a broad goal to get stronger, or a one-time gym schedule is not this Standard.",
    "Use PROTEIN_STANDARD only when the player establishes a recurring daily protein target or minimum such as 'my daily protein target is 150 grams'. Preserve the explicit gram target in attributes such as targetGrams or proteinGrams. Consuming protein, reporting today's protein intake, buying food, or asking for a recommendation is not a Standard.",
    "Recurring Standards are player-authored normative contracts. Do not infer their numeric target from general knowledge, prior habits, medical guidance, app defaults, or a Direction statement.",
